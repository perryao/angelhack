...
func (s *outlierState) outliers(scale float64) []*agent.Point {
	...
	log.Printf("Starting outliers with scale %f\n", scale)
...
func (s *outlierState) bounds(scale float64) (first, third int, lower, upper float64) {
	...
	lower = fq - iqr*scale
	upper = tq + iqr*scale
	log.Printf("bounds: first: %d, third: %d. lower: %f, upper: %f\n", first, third, lower, upper)
	return
}
...
func (s *outlierState) median(data entries) (left, right int, median float64) {
	...
	log.Printf("Medians:  left: %d, right: %d, median: %f\n", left, right, median)
	return
}
